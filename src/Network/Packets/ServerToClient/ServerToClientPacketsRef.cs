...
/// </summary>
public readonly ref struct ItemBankBalancesRef
{
    private readonly Span<byte> _data;

    /// <summary>
    /// Initializes a new instance of the <see cref="ItemBankBalancesRef"/> struct.
    /// </summary>
    /// <param name="data">The underlying data.</param>
    public ItemBankBalancesRef(Span<byte> data)
        : this(data, true)
    {
    }
    ...
    /// <param name="data">The underlying data.</param>
    /// <param name="initialize">If set to <c>true</c>, the header data is automatically initialized and written to the underlying span.</param>
    private ItemBankBalancesRef(Span<byte> data, bool initialize)
    {
        this._data = data;
        if (initialize)
        {
    ...
    /// <summary>
    /// Gets the <see cref="ItemBankBalanceEntryRef"/> of the specified index.
    /// </summary>
        public ItemBankBalanceEntryRef this[int index] => new (this._data[(6 + index * ItemBankBalanceEntryRef.Length)..]);

    /// <summary>
    /// Performs an implicit conversion from a Span of bytes to a <see cref="ItemBankBalances"/>.
    /// </summary>
    /// <param name="packet">The packet as span.</param>
    /// <returns>The packet as struct.</returns>
    public static implicit operator ItemBankBalancesRef(Span<byte> packet) => new (packet, false);

    /// <summary>
    /// Performs an implicit conversion from <see cref="ItemBankBalances"/> to a Span of bytes.
    /// </summary>
    /// <param name="packet">The packet as struct.</param>
    /// <returns>The packet as byte span.</returns>
    public static implicit operator Span<byte>(ItemBankBalancesRef packet) => packet._data; 

    /// <summary>
    /// Calculates the size of the packet for the specified count of <see cref="ItemBankBalanceEntryRef"/>.
    /// </summary>
    /// <param name="balancesCount">The count of <see cref="ItemBankBalanceEntryRef"/> from which the size will be calculated.</param>
        
    public static int GetRequiredSize(int balancesCount) => balancesCount * ItemBankBalanceEntryRef.Length + 6;


/// <summary>
/// One configured bankable item and its account-wide banked count..
/// </summary>
public readonly ref struct ItemBankBalanceEntryRef
{
    private readonly Span<byte> _data;

    /// <summary>
    /// Initializes a new instance of the <see cref="ItemBankBalanceEntryRef"/> struct.
    /// </summary>
    /// <param name="data">The underlying data.</param>
    public ItemBankBalanceEntryRef(Span<byte> data)
    {
        this._data = data;
    }
    ...
    public string Alias
    {
        get => this._data.ExtractString(7, 8, System.Text.Encoding.UTF8);
        set => this._data.Slice(7, 8).WriteString(value, System.Text.Encoding.UTF8);
    }
}
}
